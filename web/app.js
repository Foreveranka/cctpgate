...
function walletFor(chainId) {
  ...
  return state.evm && { address: state.evm, href: `${CONFIG.source.explorer}/address/${state.evm}` };
}
 ...
 * wallet that already knows Avalanche Fuji should not be asked to add it again,
 * and a wallet that does not will refuse the switch with a code that says so.
 */
async function ensureChain(provider) {
  const wanted = CONFIG.source.chainIdHex;
  ...
  try {
    ...
    if (error?.code !== 4902) throw new Error('Switch your wallet to Avalanche Fuji to continue.');
      ...
      params: [
        {
          ...
          blockExplorerUrls: [CONFIG.source.explorer],
        },
      ],
    });
  }
  ...
  try {
    if (source.family === 'stellar') {
      ...
      const result = await state.evmProvider.request({
        method: 'eth_call',
        params: [{ to: CONFIG.source.usdc, data }, 'latest'],
      });
...
function explorerFor(entry) {
  return entry.direction === 'out'
    ...
    : `${CONFIG.source.explorer}/tx/${entry.txHash}`;
}
...
async function bridge() {
  ...
  try {
    ...
    await waitForReceipt(await sendEvm(CONFIG.source.usdc, approve));

    setStatus('working', 'Burning on Avalanche…');
...
async function watchDelivery(txHash) {
  for (let i = 0; i < 120; i += 1) {
    ...
    if (status === 200 && body.delivered) {
      ...
      // out, where the hash it holds is the claim on Avalanche. It is the far side
      // whichever way you went, so the link has to follow the destination
      // rather than the name.
      const arrival = CHAINS[state.to];
      const explorer =
        arrival.family === 'stellar' ? CONFIG.stellar.explorer : CONFIG.source.explorer;
      setStatus(
        'done',
